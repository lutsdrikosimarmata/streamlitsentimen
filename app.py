...
def prepare_dataset(df):
    ...
    # Buang baris yang cleaned_review-nya kosong/hanya spasi (sesuai notebook cell 22-23)
    df = df[df['cleaned_review'].str.strip() != '']

    # drop_duplicates dilakukan setelah pembersihan, sesuai urutan di notebook (cell 26-27)
...
def train_model(df):
    ...
    # Disamakan dengan notebook: TF-IDF unigram saja (tanpa ngram_range=(1,2))
    vectorizer = TfidfVectorizer(max_features=3500)
    X = vectorizer.fit_transform(x)
    # Disamakan dengan notebook: class_weight='balanced' dan random_state=0
...
if DEFAULT_CSV.exists():
    ...
    st.error(
        f'Dataset default tidak ditemukan di `{DEFAULT_CSV}`. '
        'Pastikan file "LADS crawling data(3500).csv" berada di folder yang sama dengan app.py.'
    )